...
<#
.SYNOPSIS
    Extract Azure DevOps main branch permissions using Azure DevOps CLI
.DESCRIPTION
    This script uses the Azure DevOps CLI (az devops) and REST API to extract all 
    main branch permissions for each repository in a project and exports them to a CSV file.
.EXAMPLE
...
# Git branch permission bits (subset of repository permissions)
$branchPermissions = @{
    2 = "Read"
    4 = "Contribute"
                ...
                if ($acl.acesDictionary) {
                    ...
                    foreach ($aceKey in $aceKeys) {
                        ...
                        if ($allow -gt 0 -or $deny -gt 0) {
                            ...
                            foreach ($permission in $branchPermissions.GetEnumerator()) {
                                $bit = $permission.Key
                                $permissionName = $permission.Value
                                $access = Convert-PermissionMask -allow $allow -deny $deny -bit $bit
                                
                                # Add all permissions (including "Not Set" for branch permissions)
...
foreach ($repo in $repos) {
    ...
    Write-Host "  Default branch: $branchName" -ForegroundColor DarkGray
    
    # Build branch token
    # Branch token format: repoV2/{projectId}/{repoId}/refs/heads/{branchPath}
    $branchToken = "repoV2/$projectId/$($repo.id)/refs/heads/$branchName"
    
    try {
        # Get branch-level permissions
        Write-Host "  Fetching branch-level ACLs..." -ForegroundColor DarkGray
        $branchPerms = Process-ACLEntries `
            -token $branchToken `
        ...
        if ($branchPerms.Count -gt 0) {
            Write-Host "  ✓ Found $($branchPerms.Count) branch permission entries" -ForegroundColor Green
            $allPermissions += $branchPerms
        }
        else {
            Write-Host "  ℹ No explicit branch permissions found" -ForegroundColor DarkGray
        }
        
        # Get repository-level permissions (inherited by branches)
        Write-Host "  Fetching repository-level ACLs (inherited)..." -ForegroundColor DarkGray
        $repoToken = "repoV2/$projectId/$($repo.id)"
        $repoPerms = Process-ACLEntries `
            -token $repoToken `
            -repository $repo.name `
            -branch $branchName `
            -identityLookup $identityLookup `
        ...
        if ($repoPerms.Count -gt 0) {
            # Only add repo permissions that don't already exist at branch level
            $addedCount = 0
            foreach ($repoPerm in $repoPerms) {
                $exists = $allPermissions | Where-Object {
                    $_.Repository -eq $repoPerm.Repository -and
                    $_.Branch -eq $repoPerm.Branch -and
                    $_.IdentityDescriptor -eq $repoPerm.IdentityDescriptor -and
                    $_.Permission -eq $repoPerm.Permission
                }
                
                if (-not $exists) {
                    $allPermissions += $repoPerm
                    $addedCount++
                }
            }
            
            if ($addedCount -gt 0) {
                Write-Host "  ✓ Added $addedCount repository-level inherited entries" -ForegroundColor Green
            }
        }
        
        # Get project-level permissions (inherited by repositories and branches)
        Write-Host "  Fetching project-level ACLs (inherited)..." -ForegroundColor DarkGray
        $projectToken = "repoV2/$projectId"
        $projectPerms = Process-ACLEntries `
            -token $projectToken `
            -repository $repo.name `
            -branch $branchName `
            -identityLookup $identityLookup `
        ...
        if ($projectPerms.Count -gt 0) {
            # Only add project permissions that don't already exist
            $addedCount = 0
            foreach ($projPerm in $projectPerms) {
                $exists = $allPermissions | Where-Object {
                    $_.Repository -eq $projPerm.Repository -and
                    $_.Branch -eq $projPerm.Branch -and
            ...
            if ($addedCount -gt 0) {
                Write-Host "  ✓ Added $addedCount project-level inherited entries" -ForegroundColor Green
            }
        }
    }
    catch {
...
# Export to CSV
$outputFile = "branch-permissions.csv"
